    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        switch (event.getAction()){
            case MotionEvent.ACTION_DOWN:
                ...
                Log.i(TAG, "onInterceptTouchEvent: down dsdfsa=" + lastY);
                // 当开始滑动的时候，告诉父view
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                lastY = y;
                lastX = x;
                Log.i(TAG, "onInterceptTouchEvent: ACTION_MOVE=" + dx);

                if (Math.abs(dx)-Math.abs(dy) >= 0 && (dx == 0 && dy == 0 || Math.abs(dx) >= 50)){
                    return false;
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()){
            case MotionEvent.ACTION_MOVE:
                ...
                Log.i(TAG, "onTouchEvent: lastX=" + lastX);
