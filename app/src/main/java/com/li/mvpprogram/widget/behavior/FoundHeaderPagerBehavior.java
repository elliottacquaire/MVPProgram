...
public class FoundHeaderPagerBehavior extends ViewOffsetBehavior {
    private static final String TAG = "FoundHeadeBehavior";
    ...
    public FoundHeaderPagerBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    private void init() {
        mOverScroller = new OverScroller(BaseApplication.getInstance());
    }
    ...
    private boolean canScroll(View child, float pendingDy) {
        ...
        int pendingTranslationX = (int) (child.getTranslationX() - pendingDy);
...
//        }
        ...
        if (child.getTranslationY() >= getHeaderOffsetRange() && child.getTranslationY() < getHeaderOffsetRange()+80){
            mFlingRunnable.scrollToClosed(DURATION_SHORT);
        }
