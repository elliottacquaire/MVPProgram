...
import com.li.mvpprogram.utils.FileUtils;
...
public class AlbumBitmapCacheHelper {
    ...
    public static void init(Context context) {
        LogUtils.d("AlbumBitmapCacheHelper", "init");
    ...
    public void uninit() {
        LogUtils.d("AlbumBitmapCacheHelper", "uninit");
        ...
        this.tpe.execute(new Runnable() {
            public void run() {
                if (AlbumBitmapCacheHelper.this.currentShowString.contains(path) && AlbumBitmapCacheHelper.this.cache != null) {
                    Bitmap bitmap = null;
                    if (width != 0 && height != 0) {
                        String msg = EncryptUtils.md5(path + "_" + width + "_" + height);
    ...
    static int calculateMemoryCacheSize(Context context) {
        ...
        boolean largeHeap = (context.getApplicationInfo().flags & 1024 * 1024L) != 0;
        int memoryClass = am.getMemoryClass();
        if (largeHeap && VERSION.SDK_INT >= 11) {
            memoryClass = am.getLargeMemoryClass();
        }

        return (int) (1024 * 1024L * (long) memoryClass / 8L);
    }
